...
// 	vector<double> real_ends = {247249719.0, 242951149.0, 199501827.0, 191273063.0, 180857866.0, 170899992.0, 158821424.0, 146274826.0, 140273252.0, 135374737.0, 134452384.0, 132349534.0, 114142980.0, 106368585.0, 100338915.0, 88827254.0, 78774742.0, 76117153.0, 63811651.0, 62435964.0, 46944323.0, 49691432.0, 154913754.0}
	...
	dna_metropolis dnam(beadnr, 8.e-8, 8.e-8, 4., 290);
	
	unsigned n_connections = 5000;
	dnam.use_loops = true;
	dnam.use_gravity = false;
// 	dnam.start_big_random = true;
	dnam.init(n_chromosomes, n_beads, real_ends, band_data, n_connections*7/11, n_connections*3/11, n_connections/11);
	
	ofstream tmpfile("ranlo8_8_tmpfile.txt");
// 		cout << std::scientific << dnam.get_energy() << "\t" << dnam.get_acceptance_ratio() << "\t" << dnam.get_avg_dist() << endl;
	...
	ofstream logfile("ranlo8_8_logfile.txt");
...
//		cout << std::scientific << dnam.get_energy() << "\t" << dnam.get_acceptance_ratio() << "\t" << dnam.get_avg_dist() << endl;
		...
		//		string rname = string("results/ranlo/res_res_") + boost::lexical_cast<string>(k) + string(".txt");
 		string rname = string("/imports/lydia.work/loeb/dna_metropolis/ranlo8_8/res_res_") + boost::lexical_cast<string>(k) + string(".txt");
		...
		//		string dname = string("results/ranlo/res_distances_") + boost::lexical_cast<string>(k) + string(".txt");
 		string dname = string("/imports/lydia.work/loeb/dna_metropolis/ranlo8_8/res_distances_") + boost::lexical_cast<string>(k) + string(".txt");
