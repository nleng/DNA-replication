...
// 	argv[0] points to the program name
	std::string prozenteID = argv[1];
// 	std::string zusatzID = argv[2];
	std::string zusatzID = ""; //
	double parameter[argc-3];
	for (int i = 3; i < argc; i++)
		parameter[i-3] = strtod(argv[i], NULL);

// 	string prozenteID = "0";
	fstream infile2("chromatinTypeFac"+prozenteID+".txt");
...
// 	string zusatzID = "2";
	// g++ -std=c++0x -O3 -Wall paper_main.cpp dna_metropolis.cpp -o d0 && qsub -N d0 "./d0"
	// konstanten stimmen nicht mit denen aus daniel dissertation/paper_draft ueberein !!! k_eu = 1.e-8, k_het = 5.e-7, k_loop = 5.e-7, k_abstossung = 2.e-4
	// alte konstanten: (beadnr, 1.e-6, 3.e-8, 1.e-6, 0.0002, 290);
	// kappa_2 ist die loop-konstante
	// kappa_3 ist die abstossung der chromatin-center
	// 				n_beads, kappa_het, kappa_eu, kappa_fac, kappa_2, kappa_3, kappa_fac_rep, temperature)
	string outdir = ("/imports/debora.work/nicor/dnaMetropolisResults/00Prozente/"+prozenteID+"/");  // zusatzID
	dna_metropolis dnam(beadnr, parameter[0], parameter[1], parameter[2], parameter[3], parameter[4], parameter[5], 290);
...
//		cout << std::scientific << dnam.get_energy() << "\t" << dnam.get_acceptance_ratio() << "\t" << dnam.get_avg_dist() << endl;
		...
		if(k % 10000 == 0)
		{
			string rname = "/imports/debora.work/nicor/dnaMetropolisResults/00Prozente/" + prozenteID + zusatzID + ".txt";
			ofstream rofile(rname.c_str());
			rofile << dnam;
			rofile.close();
		}
