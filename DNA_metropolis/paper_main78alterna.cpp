...
int main(void)
{
	...
	vector<unsigned> n_beads = {2473, 2430, 1996, 1913, 1809, 1709, 1589, 1463, 1403, 1354, 1345, 1324, 1142, 1064, 1004, 889, 788, 762, 639, 625, 470, 497, 1550};
	...
	dna_metropolis dnam(beadnr, 1.e-7, 3.e-8, 1.e-7, 0.0002, 290);
...
// 	dnam.start_big_random = true;
	dnam.init(n_chromosomes, n_beads, real_ends, band_data, n_connections/4, n_connections/4, n_connections/2, true);
	
	string outdir = ("/imports/bott.work/loeb/dna_metropolis/paper_alterna78/");
...
// 		cout << std::scientific << dnam.get_energy() << "\t" << dnam.get_acceptance_ratio() << "\t" << dnam.get_avg_dist() << endl;
	...
	for(unsigned k = 0; k < 1000; ++k)
	{
	  logfile << "Started round " << k << endl;
	  	double accepted = dnam.run(100000000);
